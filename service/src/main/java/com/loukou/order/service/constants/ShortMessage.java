package com.loukou.order.service.constants;

public class ShortMessage {
    //拒单
    public static String REFUSE_MESSAGE_MODEL = "对不起,由于店主原因,订单 %s 被取消,付款金额会在15个工作日内退还给您";

    //打包完成
    public static String FINISH_PACKAGE_MESSAGE_MODEL = "您的订单 %s 已出库,配送员 %s , %s 正在飞速送来";

    //回单
    public static String FINISH_ORDER_MESSAGE_MODEL = "您的订单 %s 已签收,感谢您信任楼口.若未收到货,请拨打投诉热线400 678 0519";
    
    
    //抢摊短信
    public static String FINISH_ORDER_SOURCE_QIANGTAN_MESSAGE_MODEL_ ="";
}
